...
const getList=async ()=>{
    try{
        ...
        lists.forEach((el,index)=>{
            listcontent+=`<li class="list-group-item ${el.completed?'disabledList':''} ${index%2?'list-group-item':'list-group-item'}"> <label for="">${el.userId}</label>&nbsp&nbsp<input type="checkbox" class="checkbox" ${el.completed?' checked':''}/> <label for=""> ${el.title}</label></li>`;
        });
